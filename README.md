...
text marks schema 当前支持 `bold`、`italic`、`underline` 和 `strike` 共存，四种 boolean mark 均已完成 command、renderer、demo 和 history 闭环。第 11 周 Day 2 已完成 `8–72px` 字号闭环；Day 3 已完成文字颜色闭环，只接受 `#RGB` / `#RRGGBB` 并统一规范化为小写六位颜色，提供 `setTextColor` command、安全渲染、中文颜色控件和浏览器验收。下一步进入 Day 4「背景色」。
