...
四种 boolean mark 与字号、文字颜色、背景色已支持跨连续顶层 paragraph、heading、quote 应用和取消；command 会把范围拆成逐块 operation，并在同一 transaction 中统一状态、恢复正向或反向选区。CodeBlock、列表、表格和 void block 仍是样式范围边界，Link Mark 仍保持单块规则。
