...
text marks schema 当前支持 `bold`、`italic`、`underline` 和 `strike` 共存，helper、校验、规范化、operation 和 history 会保留四种 boolean mark；四种 mark 均已具备 command、同一 paragraph 内跨 text 的统一 `toggle_mark`、相邻同 marks text 合并、renderer 输出、demo 中文验收样例、toolbar active 状态、JSON 映射展示和 history 记录。第 10 周 Day 3「Strike」已完成，下一步进入 Day 4「快捷键占位」。
