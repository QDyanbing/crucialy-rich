...
const value = createDocument([createParagraph([createText("Hello crucialy-rich.")])]);

export function Demo() {
  return <RichTextEditor label="Editor" value={value} />;
}
```
...
- `label`、`className` 和基础 DOM 事件 props。

完整说明见 [Component API](../../docs/features/component-api.md)。
