...
- `contentEditable` 开启后，普通 `insertText` 和 collapsed selection 下的 Backspace 会通过 transaction 更新模型并触发 `onChange`。
...
当前组件仍不包含 Delete、Enter、历史、输入法完整处理、粘贴解析或序列化能力；Backspace 当前只覆盖 collapsed selection 下的段中删除和段首合并。
