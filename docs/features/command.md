...
Command 系统负责把“可执行的编辑意图”包装成统一接口。当前阶段提供注册、查询、可执行判断、按名称执行、状态读取，并内置加粗、斜体、下划线、删除线、字号、文本插入、删除选区、分段和合并段落 command。
...
- demo 操作区的“加粗”“斜体”“下划线”“删除线”“插入”“删除选区”“分段”和“合并段落”按钮，以及字号选择器，都会通过 `executeCommand` 调用 command。
