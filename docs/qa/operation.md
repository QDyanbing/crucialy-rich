...
- 普通 `beforeinput insertText` 已接入输入事件管线，删除和换行类输入尚未接入。
- 失败保护依赖 operation 不可变返回新文档。

## 结论

`insert_text`、`delete_text`、`split_block`、`merge_block` 和 transaction 的核心模型操作、测试、演示调试、摘要、闭环验收报告和 QA 记录已闭环；普通文本输入已经进入输入事件管线。
