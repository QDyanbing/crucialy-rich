...
第 7 周 Day 5「Command 闭环验收」覆盖 Command 注册、查询、状态读取、执行结果、内置文本命令、内置块命令、React 输入层复用和 demo 调试区。
...
| 状态读取   | 调整 selection 的 anchor/focus                  | Command 状态面板在可用/不可用之间同步切换       | 通过 |
...
- 当前还没有 toolbar 分组、快捷键映射、history 或 undo/redo。

## 结论

Command 系统已经完成第 7 周闭环：core 提供默认注册表和内置命令，demo 和 React 复用同一套 registry，状态面板可验证 command 可用性，自动化测试覆盖默认注册、状态矩阵、综合执行和浏览器交互。
