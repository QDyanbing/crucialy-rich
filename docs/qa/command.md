...
- `packages/core/tests/command/link.test.ts`、`cross-block-link-boundary.test.ts`：链接设置、覆盖、取消、跨块边界、状态读取、History 和安全 payload。
...
| 跨块链接     | 跨 paragraph/heading/quote 设置或取消链接       | 多条 operation 组成一个 transaction 与历史项    | 通过 |
