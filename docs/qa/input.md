...
| 选区替换       | 跨 text 或连续文本块选中文字后输入     | 先删除选区，再插入输入文字        | 通过 |
| 选区删除       | 跨 text 或连续文本块选中后按删除键     | 删除选区并折叠到起点              | 通过 |
...
输入已接入 Command、Transaction、History 和文本/void block 流程。当前已验证同容器跨 text、连续顶层文本块、CodeBlock 与 Divider 边界；结构节点之间的跨块编辑继续保持显式限制。
