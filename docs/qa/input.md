...
验证 `beforeinput insertText` 能通过 transaction 更新模型，而不是直接信任浏览器 DOM 修改结果。
...
- `tests/e2e/demo-shell.spec.ts`：演示页真实输入单字符和连续输入后，文档 JSON 与选区 JSON 同步更新。
...
- Backspace、Delete、Enter、粘贴和 IME composition 尚未接入。
- 当前不记录 undo/redo 或 history。
- 输入行为暂只覆盖 paragraph 内 text 节点。

## 结论

`beforeinput insertText` 第一版已经闭环：真实输入会转换为 transaction，更新模型并同步选区；下一步进入第 6 周 Day 2「Backspace」。
