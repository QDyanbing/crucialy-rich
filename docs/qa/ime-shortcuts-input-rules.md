...
| 格式快捷键       | Mod+B、Mod+I、Mod+U 执行对应 Command                  | 通过 |
| History 快捷键   | Mod+Z、Mod+Shift+Z 和 Mod+Y 解析为宿主 History 动作   | 通过 |
| 快捷键冲突       | 无修饰键、Alt 组合和 Composition 期间不误触发         | 通过 |
...
- `keyboard-shortcut.test.ts` 覆盖格式、撤销重做、冲突和组合输入保护。
- `input-rule/match.test.ts` 覆盖五类触发、选区限制和误触发保护。
- `input-rule/transform.test.ts` 覆盖目标结构、transaction 和转换后 selection。
- Playwright 覆盖中文组合输入、格式快捷键、撤销重做、五类输入规则、撤销和误触发。
- 完整质量门禁使用 `pnpm check:all`。

## 结论

第 23 周范围已闭环，输入法候选阶段没有中间模型写入，快捷键和输入规则都通过统一 Command、Transaction 与 History 边界工作。未提前实现第 24 周组件 API 定稿与发布配置。
