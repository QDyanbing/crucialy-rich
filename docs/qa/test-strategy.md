...
当前阶段已经包含工程冒烟测试、文档模型单测、text marks 单测、Block Type 与 operation 单测、Range/Block Selection 单测、渲染与 DOM 映射单测、React 组件 API 单测、输入 helper 单测、command 单测、history 单测、工具栏、斜杠菜单与图片单测和演示端到端验收。
...
- 工具：Playwright。
- 范围：演示页面、模型/选区/command 调试、基础编辑、文字样式、链接、Heading/Quote、CodeBlock、Divider、列表、固定/悬浮 Toolbar、斜杠菜单、URL/本地图片、图片删除、选区恢复和 History 往返。
- 命令：`pnpm test:e2e`。
