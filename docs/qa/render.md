...
验证模型到渲染树、bold/italic mark 渲染、HTML 序列化、路径属性绑定、DOM 与模型映射、React 组件渲染和演示渲染入口。
...
- `packages/core/tests/render/render.test.ts`：document、paragraph、text、bold mark、italic mark、bold+italic 组合、空 document 和空 paragraph 的渲染树结构。
- `packages/core/tests/render/html.test.ts`：HTML 序列化、bold mark、italic mark、bold+italic 组合序列化和文本转义。
...
- 暂不实现真实编辑输入。
- 当前只包含 bold 和 italic 标记渲染，暂不包含标题、列表等扩展节点渲染。
